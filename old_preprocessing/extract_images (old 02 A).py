...
IMAGES_PER_TOPIC_DIR = resolve_env_path("IMAGES_PER_TOPIC_DIR", must_exist=False)
...
def analyze_missing_messages(
    ...
    for topic in image_topics:
        ...
        existing_files = set()
        if topic_dir.exists():
            existing_files = {p.stem for p in topic_dir.glob("*.webp")}
...
# Saves the image to a file, raising if OpenCV fails to persist it.
def save_image(img, img_filepath: Path) -> None:
    success = cv2.imwrite(str(img_filepath), img)
    if not success:
        raise IOError(f"cv2.imwrite returned False for {img_filepath}")

# Extracts and saves an image from a single message
def extract_image_from_message(msg, topic, timestamp, output_dir: Path):
...
def extract_images_from_rosbag(rosbag_path: Path, output_dir: Path, csv_path: Path, format: str):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        if format == 'db3':
            with Reader(str(rosbag_path)) as reader:
                ...
                for connection, timestamp, rawdata in tqdm(reader.messages(connections=connections), desc=f"Extracting {rosbag_path.name}"):
                    ...
                    if (topic_dir / f"{timestamp_str}.webp").exists():
                    ...
                    try:
                        ...
                        print(f"\nError deserializing message from {topic} at {timestamp}: {exc}")
                        if missing_set is not None:
                            missing_set.add(timestamp_str)
                        else:
        ...
        for future in futures:
            ...
            if topic and timestamp_str:
                topic_dir = output_dir / topic_dir_map[topic]
                img_path = topic_dir / f"{timestamp_str}.webp"
                if not img_path.exists():
                    print(f"Warning: expected image {img_path} missing after extraction")
                    if topic in missing_timestamp_map:
                        missing_timestamp_map[topic].add(timestamp_str)
                    else:
                        scheduled_full_scan.setdefault(topic, set()).discard(timestamp_str)
...
def main():
    ...
    images_root = IMAGES_PER_TOPIC_DIR
