...
It extracts timestamps using log_time from MCAP messages, determines a reference topic (highest frequency),
...
LOOKUP_TABLES_STR = "/metadata/lookup_tables_new"
...
SKIP: List[str] = []
MAX_WORKERS: int = 2  # Reduced to avoid terminal output contention
...
def extract_timestamp(
    ...
    """Extract timestamp using log_time from MCAP.
    
    Args:
        topic: Topic name
        data: Serialized message data (unused, kept for compatibility)
        log_time: Log time from MCAP
...
def process_single_mcap(mcap_path: Path) -> Tuple[Dict[str, List[int]], Dict[str, str]]:
    ...
    try:
        ...
        # Get topic types for topic type tracking
        topic_type_map: Dict[str, str] = {}
        try:
        ...
        while reader.has_next():
            ...
            # Use log_time directly, no need to deserialize messages
