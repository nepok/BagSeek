...
def select_representative_preview_mcaps(mcap_files: List[Path]) -> List[int]:
    """Select 7 MCAPs for representative previews: first MCAP of parts 2-8.
    
    Args:
        mcap_files: List of MCAP file paths, sorted numerically
        
    Returns:
        List of MCAP numbers (as integers) selected for representative previews
    """
    if len(mcap_files) < 8:
        # If less than 8 MCAPs, just take the middle ones
        if len(mcap_files) <= 1:
            return []
        step = max(1, len(mcap_files) // 8)
        selected_indices = [step * i for i in range(1, min(8, len(mcap_files)))]
        return [extract_mcap_number(mcap_files[i]) for i in selected_indices if i < len(mcap_files)]
    
    # Split into 8 parts, take first MCAP of parts 2-8
    num_mcaps = len(mcap_files)
    part_size = num_mcaps / 8.0
    selected_mcap_numbers = []
    
    for i in range(1, 8):  # Parts 2-8
        mcap_idx = int(i * part_size)
        if mcap_idx < num_mcaps:
            mcap_number = extract_mcap_number(mcap_files[mcap_idx])
            selected_mcap_numbers.append(mcap_number)
    
    return selected_mcap_numbers

def extract_data_from_single_mcap(mcap_path: Path, selected_for_preview: bool = False) -> Tuple[Dict[str, List[int]], Dict[str, str], List[Tuple[str, bytes, int, str]], List[Tuple[str, float, float, int, str]], Dict[str, Tuple[str, bytes, int, str]]]:
    """Extract data from a single MCAP file.
    
    Args:
        mcap_path: Path to the MCAP file
        selected_for_preview: If True, cache first image per topic for representative previews
        
    Returns:
        Tuple of (topic_data, topic_types, image_data_list, positional_data_list, preview_cache)
        topic_data: Dict mapping topic to list of timestamps
        topic_types: Dict mapping topic to type string
        image_data_list: List of (topic, bytes, timestamp_ns, mcap_identifier)
        positional_data_list: List of (topic, lat, lon, timestamp_ns, mcap_identifier)
        preview_cache: Dict mapping topic to (topic, bytes, timestamp_ns, mcap_identifier) for first image per topic
    """
    mcap_number = extract_mcap_number(mcap_path)
    mcap_identifier = str(mcap_number)
    ...
    topic_data: Dict[str, List[int]] = defaultdict(list)
    topic_types: Dict[str, str] = {}
    image_data_list: List[Tuple[str, bytes, int, str]] = []
    positional_data_list: List[Tuple[str, float, float, int, str]] = []
    preview_cache: Dict[str, Tuple[str, bytes, int, str]] = {}  # topic -> first image
                ...
                if schema_name in ("sensor_msgs/msg/CompressedImage", "sensor_msgs/msg/Image"):
                    if hasattr(ros2_msg, 'data'):
                        ...
                        if image_bytes:
                            ...
                            # Cache first image per topic for representative previews
                            if selected_for_preview and topic not in preview_cache:
                                preview_cache[topic] = (topic, image_bytes, log_time, mcap_identifier)
                
                # Extract positional data if BESTPOS topic
                if schema_name == "novatel_oem7_msgs/msg/BESTPOS":
                    if hasattr(ros2_msg, 'lat') and hasattr(ros2_msg, 'lon'):
                        try:
                            lat = float(ros2_msg.lat)
    ...
    return topic_data, topic_types, image_data_list, positional_data_list, preview_cache

def create_lookup_table_for_mcap(mcap_number: int, topic_data: Dict[str, List[int]], topic_types: Dict[str, str], csv_path: Path, all_topics: Optional[List[str]], rosbag_name: str) -> bool:
    """Create lookup table CSV file for a single MCAP.
    
    Args:
        mcap_number: MCAP number identifier
        topic_data: Dict mapping topic to list of timestamps
        topic_types: Dict mapping topic to type string
        csv_path: Base CSV path (will determine directory structure)
        all_topics: Optional list of all topics
        rosbag_name: Name of the rosbag
        
    Returns:
        True if created successfully, False otherwise
    """
    parent_name = Path(csv_path).parent.name if csv_path.parent != csv_path.parent.parent else ""
    ...
    try:
        ...
        logger.error(f"    ❌ Error creating lookup table for MCAP {mcap_number}: {e}")
        return False
...
def sanitize_topic_name(topic: str) -> str:
    ...
    Removes leading slashes and replaces remaining slashes with double underscores.
    This ensures topic names like "/camera/front/image_raw" become safe directory names.
    
    Args:
        topic: Topic name (e.g., "/camera/front/image_raw")
        
    Returns:
        Sanitized topic name (e.g., "camera__front__image_raw")
    """
...
def get_tmp_embedding_dir(rosbag_name: str, mcap_id: str, model_id: str, topic: str) -> Path:
    """Get tmp directory path for storing embeddings per MCAP.
    
    Returns:
        Path to tmp/{rosbag_name}/{mcap_id}/{model_id}/{sanitized_topic}/
    """
    if not EMBEDDINGS:
        raise ValueError("EMBEDDINGS not configured")
    sanitized_topic = sanitize_topic_name(topic)
    return EMBEDDINGS / "tmp" / rosbag_name / str(mcap_id) / model_id / sanitized_topic

def write_embeddings_to_tmp(embeddings: List[Tuple[str, np.ndarray, int, str]], rosbag_name: str, mcap_id: str, model_id: str) -> bool:
    """Write embeddings to tmp folder per MCAP.
    
    Args:
        embeddings: List of (topic, embedding_array, timestamp_ns, mcap_identifier)
        rosbag_name: Name of the rosbag
        mcap_id: MCAP identifier (string)
        model_id: Model identifier
    ...
    try:
        ...
        embeddings_by_topic: Dict[str, List[Tuple[np.ndarray, int, str]]] = defaultdict(list)
        for topic, embedding, timestamp_ns, mcap_id_val in embeddings:
            embeddings_by_topic[topic].append((embedding, timestamp_ns, mcap_id_val))
        
        # Write each topic's embeddings
        for topic, emb_list in embeddings_by_topic.items():
            tmp_dir = get_tmp_embedding_dir(rosbag_name, mcap_id, model_id, topic)
            tmp_dir.mkdir(parents=True, exist_ok=True)
            
            # Save embeddings as numpy array
            embeddings_array = np.stack([emb for emb, _, _ in emb_list]).astype(OUTPUT_DTYPE)
            embeddings_path = tmp_dir / "embeddings.npy"
            np.save(embeddings_path, embeddings_array)
            
            # Save metadata
            metadata = [{"topic": topic, "timestamp_ns": ts, "mcap_identifier": mcap_id_val} 
                       for _, ts, mcap_id_val in emb_list]
        ...
        logger.error(f"      ❌ Error writing embeddings to tmp for {model_id}/{rosbag_name}/{mcap_id}: {e}")
        import traceback
        logger.debug(traceback.format_exc())
        return False

def load_embeddings_from_tmp(rosbag_name: str, model_id: str) -> Dict[str, List[Tuple[np.ndarray, int, str]]]:
    """Load all embeddings from tmp folders for a rosbag/model combination.
    ...
    tmp_base = EMBEDDINGS / "tmp" / rosbag_name
    ...
    try:
        # Iterate through all MCAP directories
        for mcap_dir in tmp_base.iterdir():
            if not mcap_dir.is_dir():
                continue
            
            model_dir = mcap_dir / model_id
            ...
            for topic_dir in model_dir.iterdir():
                ...
                embeddings_path = topic_dir / "embeddings.npy"
                ...
                # Get original topic name from metadata (for backwards compatibility)
                # The metadata stores the original topic name, not the sanitized directory name
                original_topic = metadata[0].get("topic", topic_dir.name)
                ...
                for i, meta in enumerate(metadata):
                    if i < len(embeddings_array):
                        ...
                        mcap_id = meta["mcap_identifier"]
                        embeddings_by_topic[original_topic].append((embedding, timestamp_ns, mcap_id))
        
        # Sort each topic's embeddings by timestamp
        for topic in embeddings_by_topic:
            embeddings_by_topic[topic].sort(key=lambda x: x[1])
        
        total_embeddings = sum(len(embs) for embs in embeddings_by_topic.values())
        logger.info(f"      ✓ Loaded {total_embeddings:,} embeddings from tmp for {model_id}/{rosbag_name}")
        
        return dict(embeddings_by_topic)
        
    except Exception as e:
        logger.warning(f"      ⚠️  Error loading embeddings from tmp: {e}")
        import traceback
        logger.debug(traceback.format_exc())
        return {}

def cleanup_tmp_embeddings(rosbag_name: str):
    """Clean up tmp embedding folders for a rosbag.
    ...
    tmp_dir = EMBEDDINGS / "tmp" / rosbag_name
    ...
    try:
        ...
        logger.debug(f"      Cleaned up tmp embeddings for {rosbag_name}")
    except Exception as e:
        logger.warning(f"      Failed to cleanup tmp embeddings for {rosbag_name}: {e}")

def is_mcap_embeddings_completed(rosbag_name: str, mcap_id: str, model_id: str) -> bool:
    """Check if embeddings are completed for a specific MCAP/model combination.
    ...
    Returns:
        True if embeddings exist in tmp folder for this MCAP (at least one topic)
    """
    if not EMBEDDINGS:
        return False
    
    tmp_base = EMBEDDINGS / "tmp" / rosbag_name / str(mcap_id) / model_id
    if not tmp_base.exists():
        return False
    
    # Check if at least one topic has embeddings
    for topic_dir in tmp_base.iterdir():
        if topic_dir.is_dir() and (topic_dir / "embeddings.npy").exists():
            return True
...
def process_mcap_images(image_data_list: List[Tuple[str, bytes, int, str]], mcap_id: str, preprocessing_groups: Dict[str, List[str]], rosbag_name: str) -> bool:
    """Process images from a single MCAP: convert, preprocess, embed, and write to tmp.
    
    Args:
        image_data_list: List of (topic, bytes, timestamp_ns, mcap_identifier) for this MCAP
        mcap_id: MCAP identifier (string)
        preprocessing_groups: Dict mapping preprocessing string to list of model IDs
        rosbag_name: Name of the rosbag
    ...
    Returns:
        True if successful, False otherwise
    """
    if not EMBEDDINGS or not image_data_list:
        return True
    
    logger.info(f"      📸 Processing {len(image_data_list):,} images from MCAP {mcap_id}...")
    
    # Convert images to PIL
    pil_images = convert_images_to_pil(image_data_list)
    del image_data_list
    gc.collect()
    
    if not pil_images:
        logger.warning(f"      ⚠️  No valid PIL images from MCAP {mcap_id}")
        return False
    ...
    for preprocess_str, model_ids in preprocessing_groups.items():
        ...
        preprocess_transform = None
        first_model_id = model_ids[0] if model_ids else None
        ...
        if not preprocessed_tensors:
            logger.warning(f"      ⚠️  No preprocessed tensors for preprocessing group {first_model_id}")
            continue
        
        # Generate embeddings for each model in this group
        for model_id in model_ids:
            # Check if already completed
            if is_mcap_embeddings_completed(rosbag_name, mcap_id, model_id):
                logger.debug(f"        ⏭️  {model_id}: Embeddings already completed for MCAP {mcap_id}, skipping")
                continue
            ...
            # Write embeddings to tmp
            if embeddings:
                success = write_embeddings_to_tmp(embeddings, rosbag_name, mcap_id, model_id)
                if success:
                    logger.debug(f"        ✓ {model_id}: Wrote {len(embeddings):,} embeddings to tmp")
                else:
                    logger.error(f"        ❌ {model_id}: Failed to write embeddings to tmp")
...
def finalize_rosbag_processing(rosbag_name: str, preprocessing_groups: Dict[str, List[str]], preview_cache: Dict[str, List[Tuple[str, bytes, int, str]]]) -> None:
    """Finalize rosbag processing: adjacent similarities, sharding, representative previews, cleanup.
    
    Args:
        rosbag_name: Name of the rosbag
        preprocessing_groups: Dict mapping preprocessing string to list of model IDs
        preview_cache: Dict mapping topic to list of (topic, bytes, timestamp_ns, mcap_identifier) for representative previews
    """
    if not EMBEDDINGS:
        return
    
    logger.info(f"  🔄 Finalizing rosbag processing for {rosbag_name}...")
    
    # Collect all model IDs
    all_model_ids = []
    for model_ids in preprocessing_groups.values():
        all_model_ids.extend(model_ids)
    all_model_ids = list(set(all_model_ids))
    
    # Load embeddings from tmp and process
    all_embeddings: Dict[str, Dict[str, List[Tuple[np.ndarray, int, str]]]] = {}
    
    for model_id in all_model_ids:
        # Load embeddings from tmp
        model_embeddings = load_embeddings_from_tmp(rosbag_name, model_id)
        if model_embeddings:
            all_embeddings[model_id] = model_embeddings
    
    # Compute adjacent similarities
    if ADJACENT_SIMILARITIES and all_embeddings:
        logger.info(f"  🔍 Computing adjacent similarities...")
        for model_id, model_embeddings in all_embeddings.items():
            similarity_topics = []
            similarity_errors = []
    ...
    if all_embeddings:
        try:
            create_shards(all_embeddings, rosbag_name, EMBEDDINGS)
            # Mark shard creation as completed for each model
            for model_id in all_embeddings.keys():
                mark_embeddings_completed(rosbag_name, model_id, "shard_creation", "success")
            logger.info(f"  ✓ Created shards for {len(all_embeddings)} model(s)")
        except Exception as e:
            error_msg = str(e)
            logger.error(f"  ❌ Error creating shards: {e}")
            for model_id in all_embeddings.keys():
    ...
    # Create representative previews
    if preview_cache and REPRESENTATIVE_PREVIEWS:
        # preview_cache is Dict[topic] -> List[Tuple[...]]
        # We need to take first image per topic (or all if we want more)
        preview_images = []
        for topic, images in preview_cache.items():
            if images:
                # Take first image from each selected MCAP (should be 7 total)
                preview_images.extend(images)
        
        if preview_images:
            create_representative_previews(preview_images, rosbag_name)
    
    # Cleanup tmp folders
    cleanup_tmp_embeddings(rosbag_name)
    logger.info(f"  ✓ Finalization complete for {rosbag_name}")
...
def process_single_rosbag(pipeline: RosbagPipeline, preprocessing_groups: Dict[str, List[str]]) -> List[Tuple[str, float, float, int, str]]:
    """Process a single rosbag based on pipeline configuration (per-MCAP processing).
    ...
    try:
        # =================================================================
        # SETUP PHASE: Per-rosbag initialization
        # =================================================================
        mcap_files = find_mcap_files(pipeline.rosbag_path)
        if not mcap_files:
            logger.warning(f"  ⚠️  No MCAP files found in {rosbag_name}")
            return []
        
        # Select MCAPs for representative previews (parts 2-8)
        selected_mcap_numbers = select_representative_preview_mcaps(mcap_files)
        selected_mcap_set = set(selected_mcap_numbers)
        logger.debug(f"  Selected {len(selected_mcap_numbers)} MCAP(s) for representative previews: {selected_mcap_numbers}")
        
        # Initialize aggregated data
        all_topic_types: Dict[str, str] = {}
        all_positional_data: List[Tuple[str, float, float, int, str]] = []
        preview_cache: Dict[str, List[Tuple[str, bytes, int, str]]] = defaultdict(list)  # topic -> list of first images
        
        # Get all topics from metadata for lookup tables
        all_topics = get_all_topics_from_metadata(pipeline.rosbag_path)
        
        # =================================================================
        # PER-MCAP PROCESSING LOOP
        # =================================================================
        logger.info(f"  🔄 Processing {len(mcap_files)} MCAP(s)...")
        
        for mcap_path in tqdm(mcap_files, desc=f"  Processing MCAPs"):
            mcap_number = extract_mcap_number(mcap_path)
            mcap_identifier = str(mcap_number)
            is_selected_for_preview = mcap_number in selected_mcap_set
            
            try:
                # Check if this MCAP needs processing
                needs_processing = False
                if pipeline.needs_lookup_tables:
                    # Check if lookup table exists
                    parent_name = pipeline.rosbag_path.parent.name
                    if parent_name.endswith("_multi_parts"):
                        csv_mcap_dir = pipeline.csv_path.parent / parent_name / rosbag_name
                    else:
                        csv_mcap_dir = pipeline.csv_path.parent / rosbag_name
                    csv_path_final = csv_mcap_dir / f"{mcap_number}.csv"
                    if not csv_path_final.exists():
                        needs_processing = True
                
                if pipeline.needs_embeddings and EMBEDDINGS:
                    # Check if embeddings exist for at least one model
                    for model_ids in preprocessing_groups.values():
                        for model_id in model_ids:
                            if not is_mcap_embeddings_completed(rosbag_name, mcap_identifier, model_id):
                                needs_processing = True
                                break
                        if needs_processing:
                            break
                
                if not needs_processing and not pipeline.needs_positional_data and not is_selected_for_preview:
                    logger.debug(f"    ⏭️  MCAP {mcap_number}: Already processed, skipping")
                    continue
                
                # Extract data from this MCAP
                topic_data, topic_types, image_data_list, positional_data_list, mcap_preview_cache = extract_data_from_single_mcap(
                    mcap_path, selected_for_preview=is_selected_for_preview
                )
                
                # Merge topic types
                all_topic_types.update(topic_types)
                
                # Merge positional data
                all_positional_data.extend(positional_data_list)
                
                # Merge preview cache
                for topic, preview_image in mcap_preview_cache.items():
                    preview_cache[topic].append(preview_image)
                
                # Create lookup table for this MCAP
                if pipeline.needs_lookup_tables and topic_data:
                    success = create_lookup_table_for_mcap(
                        mcap_number, topic_data, topic_types, pipeline.csv_path, all_topics, rosbag_name
                    )
                    if success:
                        logger.debug(f"    ✓ MCAP {mcap_number}: Lookup table created")
                
                # Process images for this MCAP (if needed)
                if pipeline.needs_embeddings and EMBEDDINGS and image_data_list:
                    process_mcap_images(image_data_list, mcap_identifier, preprocessing_groups, rosbag_name)
                
                # Clear MCAP data from memory
                del topic_data, image_data_list, mcap_preview_cache
                gc.collect()
                
            except Exception as e:
                logger.error(f"    ❌ Error processing MCAP {mcap_number}: {e}")
                import traceback
                logger.debug(traceback.format_exc())
                continue  # Continue with next MCAP
        
        # =================================================================
        # FINALIZATION PHASE: Per-rosbag operations
        # =================================================================
        
        # Create topics.json
        if pipeline.needs_topics_json and pipeline.topics_json_path and all_topic_types:
            logger.info(f"  📊 Creating topics.json...")
            try:
                pipeline.topics_json_path.parent.mkdir(parents=True, exist_ok=True)
                topics = sorted(all_topic_types.keys())
                with open(pipeline.topics_json_path, 'w') as f:
                    json.dump({"topics": topics, "types": all_topic_types}, f, indent=2)
        ...
        # Update positional lookup table
        if pipeline.needs_positional_data and all_positional_data:
            logger.info(f"  📊 Creating positional lookup tables...")
            try:
                update_positional_lookup_table(rosbag_name, all_positional_data)
                mark_positional_lookup_table_completed(rosbag_names=[rosbag_name])
            except Exception as e:
                error = str(e)
        ...
        # Mark lookup tables as completed (if we created any)
        if pipeline.needs_lookup_tables:
            mcap_numbers = [extract_mcap_number(mcap) for mcap in mcap_files]
            mark_lookup_tables_completed(rosbag_name, mcap_numbers)
        
        # Finalize: adjacent similarities, sharding, representative previews, cleanup
        if (pipeline.needs_embeddings or pipeline.needs_adjacent_similarities) and EMBEDDINGS:
            finalize_rosbag_processing(rosbag_name, preprocessing_groups, preview_cache)
