#!/usr/bin/env python3
"""
Unified script for image preprocessing, embedding generation, and shard creation.

Combines functionality from:
- 03_preprocess_images.py: Image preprocessing
- 04A_create_open_clip_embeddings.py: OpenCLIP embedding generation
- 04B_create_not_open_clip_embeddings.py: Custom CLIP embedding generation
- 05_create_manifests.py: Shard and manifest creation
"""
...
SAVE_PREPROCESSED = True  # Save preprocessed .pt files to disk
SAVE_INDIVIDUAL_EMBEDDINGS = True  # Save individual embedding .pt files
SAVE_SHARDED_EMBEDDINGS = True  # Always save final sharded format
...
def get_completion_file() -> Path:
    """Get the completion file path for embedding generation."""
...
def save_completed_step(rosbag_name: str, model_id: str, step: str, status: str = "success", error: Optional[str] = None):
    ...
    # Check if all required steps are completed for this model
    required_steps = []
    if SAVE_PREPROCESSED and PREPROCESSED:
        required_steps.append("preprocessing")
    if SAVE_INDIVIDUAL_EMBEDDINGS and SINGLE_EMBEDDINGS:
        required_steps.append("individual_embeddings")
    if SAVE_SHARDED_EMBEDDINGS:
        required_steps.append("sharded_embeddings")
    
    model_steps = completed[rosbag_name]["models"][model_id]["steps"]
    all_steps_completed = all(
        step_name in model_steps and model_steps[step_name].get("status") in ("success", "skipped")
        for step_name in required_steps
    )
    completed[rosbag_name]["models"][model_id]["all_steps_completed"] = all_steps_completed
...
def save_completed_model(rosbag_name: str, model_id: str, status: str = "success", error: Optional[str] = None):
    ...
    # Mark all steps as completed with the given status
    if "steps" not in completed[rosbag_name]["models"][model_id]:
        completed[rosbag_name]["models"][model_id]["steps"] = {}
    ...
    for step in required_steps:
        if step not in completed[rosbag_name]["models"][model_id]["steps"]:
            completed[rosbag_name]["models"][model_id]["steps"][step] = {
                "status": status,
                "completed_at": timestamp
            }
...
def process_rosbag(rosbag_path: Path, rosbag_name: str) -> None:
    ...
    for preprocess_str, model_list in preprocess_dict.items():
        ...
        # Preprocess images helper function - will be called in chunks to avoid OOM
        ...
        def preprocess_single_image(args):
            ...
            try:
                    ...
                    if output_path.exists():
                        try:
                            ...
                            try:
                                ...
                                pass  # Ignore errors deleting corrupted file
                ...
                # Log error but don't crash - return None to skip this image
        ...
        for model_id_str in model_list:
            ...
            # Check for existing embeddings in manifest and individual files
            already_seen = set()
            if SAVE_SHARDED_EMBEDDINGS:
            ...
            # Also check individual embedding files if saving them
            if SAVE_INDIVIDUAL_EMBEDDINGS and SINGLE_EMBEDDINGS:
            ...
            if not images_to_process:
                ...
                if SAVE_PREPROCESSED and PREPROCESSED:
                    save_completed_step(rosbag_name, model_dir_id, "preprocessing", status="skipped", error="All images already processed")
                if SAVE_INDIVIDUAL_EMBEDDINGS and SINGLE_EMBEDDINGS:
                    save_completed_step(rosbag_name, model_dir_id, "individual_embeddings", status="skipped", error="All images already processed")
                if SAVE_SHARDED_EMBEDDINGS:
                    save_completed_step(rosbag_name, model_dir_id, "sharded_embeddings", status="skipped", error="All images already processed")
                continue
            ...
            preprocessing_done = is_step_completed(rosbag_name, model_dir_id, "preprocessing") if (SAVE_PREPROCESSED and PREPROCESSED) else True
            individual_embeddings_done = is_step_completed(rosbag_name, model_dir_id, "individual_embeddings") if (SAVE_INDIVIDUAL_EMBEDDINGS and SINGLE_EMBEDDINGS) else True
            sharded_embeddings_done = is_step_completed(rosbag_name, model_dir_id, "sharded_embeddings") if SAVE_SHARDED_EMBEDDINGS else True
            
            # If all steps are done, skip this model
            if preprocessing_done and individual_embeddings_done and sharded_embeddings_done:
                log(f"      ⏭️  All steps already completed for {model_dir_id}")
                continue
            
            # Initialize shard writing if needed (always initialize to check existing shards)
            manifest = None
            meta = None
            initial_count = 0
            out_dir = None
            ...
            # Special case: If individual embeddings exist but shards don't, load embeddings from individual files
            if individual_embeddings_done and not sharded_embeddings_done and SAVE_INDIVIDUAL_EMBEDDINGS and SINGLE_EMBEDDINGS:
            ...
            for chunk_idx in tqdm(range(num_chunks), desc=f"Chunks ({model_dir_id})", leave=False):
                ...
                # Preprocess this chunk (or load if already done)
                if preprocessing_done:
                    # Load preprocessed files instead of preprocessing
                    chunk_preprocessed = {}
                    for image_path, topic_name, timestamp_ns in chunk_images:
                        preprocessed_path = PREPROCESSED / rosbag_name / topic_name.replace("/", "__") / f"{timestamp_ns}.pt"
                        if preprocessed_path.exists():
                            try:
                                ...
                                chunk_preprocessed = preprocess_chunk(chunk_images)
                                preprocessing_done = False  # Mark as not done so we process it
                                break
                else:
                ...
                if SAVE_INDIVIDUAL_EMBEDDINGS and SINGLE_EMBEDDINGS and not individual_embeddings_done:
                ...
                # Write shards incrementally for this chunk
                if SAVE_SHARDED_EMBEDDINGS and not sharded_embeddings_done:
                    chunk_records = collect_embedding_records(chunk_embeddings, already_seen)
                    if chunk_records:
                        df_append, meta = write_shards_from_embeddings(out_dir, chunk_records, meta, DEFAULT_SHARD_ROWS, OUTPUT_DTYPE)
                        if not df_append.empty:
                            manifest = pd.concat([manifest, df_append], ignore_index=True)
                            # Don't sort yet - will sort at the end
            ...
            # Finalize shards - sort manifest and save
            if SAVE_SHARDED_EMBEDDINGS and not sharded_embeddings_done:
                if not manifest.empty:
                    manifest.sort_values("id", inplace=True, kind="mergesort")
                    write_manifest(manifest_path, manifest)
                    ...
                    new_rows = meta.get("total_count", 0) - initial_count
                    log(f"      ✅ {model_dir_id}: +{new_rows} row(s), total={meta['total_count']}")
                    # Mark sharded embeddings step as completed
                    save_completed_step(rosbag_name, model_dir_id, "sharded_embeddings", status="success")
                else:
                    log(f"      ⏭️  No new embeddings to add to shards")
                    # Check if shards already exist (from previous run)
                    if (out_dir / "shards").exists() and any((out_dir / "shards").glob("shard-*.npy")):
                        save_completed_step(rosbag_name, model_dir_id, "sharded_embeddings", status="success")
            ...
            # Mark preprocessing step as completed (shared across models in preprocessing group)
            if SAVE_PREPROCESSED and PREPROCESSED and not preprocessing_done:
    ...
    for config in CUSTOM_MODELS:
        ...
        if not checkpoint_path.exists():
            ...
            if SAVE_PREPROCESSED and PREPROCESSED:
                save_completed_step(rosbag_name, model_dir_name, "preprocessing", status="failed", error=f"Checkpoint not found: {checkpoint_path}")
            if SAVE_INDIVIDUAL_EMBEDDINGS and SINGLE_EMBEDDINGS:
            ...
            if SAVE_SHARDED_EMBEDDINGS:
                save_completed_step(rosbag_name, model_dir_name, "sharded_embeddings", status="failed", error=f"Checkpoint not found: {checkpoint_path}")
            continue

        # Check which steps are already completed
        preprocessing_done = is_step_completed(rosbag_name, model_dir_name, "preprocessing") if (SAVE_PREPROCESSED and PREPROCESSED) else True
        individual_embeddings_done = is_step_completed(rosbag_name, model_dir_name, "individual_embeddings") if (SAVE_INDIVIDUAL_EMBEDDINGS and SINGLE_EMBEDDINGS) else True
        sharded_embeddings_done = is_step_completed(rosbag_name, model_dir_name, "sharded_embeddings") if SAVE_SHARDED_EMBEDDINGS else True
        
        # If all steps are done, skip this model
        if preprocessing_done and individual_embeddings_done and sharded_embeddings_done:
            log(f"  ⏭️  {model_dir_name} - all steps already completed (skipping)")
            continue
        ...
        if not images_to_process:
            ...
            # Mark all steps as skipped
            if SAVE_PREPROCESSED and PREPROCESSED:
                save_completed_step(rosbag_name, model_dir_name, "preprocessing", status="skipped", error="All images already processed")
            if SAVE_INDIVIDUAL_EMBEDDINGS and SINGLE_EMBEDDINGS:
                save_completed_step(rosbag_name, model_dir_name, "individual_embeddings", status="skipped", error="All images already processed")
            if SAVE_SHARDED_EMBEDDINGS:
                save_completed_step(rosbag_name, model_dir_name, "sharded_embeddings", status="skipped", error="All images already processed")
            continue
        ...
        # Mark preprocessing step as completed (for custom models, preprocessing happens during embedding generation)
        if SAVE_PREPROCESSED and PREPROCESSED and not preprocessing_done:
            save_completed_step(rosbag_name, model_dir_name, "preprocessing", status="success")
...
def main():
    ...
    log("🚀 Starting embedding generation pipeline")
