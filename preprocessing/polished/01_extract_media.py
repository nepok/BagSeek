...
- Each extraction type (image, gps, video, pointcloud, odom) has its own completion file
...
POINTCLOUDS = os.getenv("POINTCLOUDS")
...
EXTRACTION_TYPES = ["image", "gps", "pointcloud", "odom"]  # List of extraction types: "image", "gps", "video", "pointcloud", "odom"
#EXTRACTION_TYPES = ["gps"]
...
TOPIC_SEARCH_TERMS: Dict[str, List[str]] = {
    ...
    "gps": ["novatel"],           # Dummy - edit as needed
...
def save_completed_mcap(rosbag_name: str, mcap_identifier: str, extraction_type: str, 
    ...
    Args:
        ...
        extraction_type: Type of extraction (image, gps, video, etc.)
...
def verify_completion(rosbag_name: str, extraction_type: str) -> bool:
    ...
    if extraction_type == "gps" or extraction_type == "odom":
        # For gps/odom: Check if rosbag directory exists and has CSV files (structure: rosbag/topic/{mcap}.csv)
...
def build_topic_config(topic: str, extraction_type: str, rosbag_name: str, mcap_identifier: Optional[str] = None) -> dict:
    ...
    if extraction_type == "gps" or extraction_type == "odom":
        # For gps/odom: rosbag/topic/{mcap}.csv structure
        ...
        # For images/pointcloud: rosbag/topic/{mcap}/ structure
        bag_out = output_dir / rosbag_name
        return {
            "format": format_str,
            "path": str(bag_out),
            "subfolder": f"%name/{mcap_identifier}" if mcap_identifier else "%name",
            "naming": "%timestamp",
        }
    elif extraction_type == "video":
        # For video: rosbag/topic/{mcap}/ structure (one video per mcap)
        bag_out = output_dir / rosbag_name
        return {
            "format": format_str,
            "path": str(bag_out),
...
def main():
    ...
    try:
        ...
        valid_types = {"image", "gps", "video", "pointcloud", "odom"}
